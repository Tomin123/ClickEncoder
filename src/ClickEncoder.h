...
#define ENC_ISR	 (1 << 3) // use interrupt on one pin (encoder "clock")
...
class ClickEncoder {
	...
	public:
		ClickEncoder(int8_t A, int8_t B, int8_t BTN = -1, uint8_t stepsPerNotch = 4,
						 bool active = LOW);

#ifndef WITHOUT_BUTTON
		explicit ClickEncoder(int8_t BTN,
									 bool active = false); // Depricated.  Use Digtial Button instead
...
class AnalogButton : public ClickEncoder {
	public:
		explicit AnalogButton(
				  int8_t BTN, int16_t rangeLow,
				  int16_t rangeHigh); // Constructor for using analog input range as a button
};

class DigitalButton : public ClickEncoder {
	public:
		explicit DigitalButton(int8_t BTN,
									  bool active = false); // Constructor for using a button only
};
